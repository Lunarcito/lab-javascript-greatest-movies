...
function getAllDirectors(moviesArray) {
  ...
  const newArray = moviesArray.map ((elemento) => elemento.director)
  //.filter ((elemento)=> elemento.director);
  //const UniqueDirectors = moviesArray.filter((el,i,array) => array.indexof(el)===i);
  return newArray
...
function howManyMovies(dramaMovies) {
    if (!dramaMovies[0]){ //.length
        return 0
    } 
    let Spieldramas= dramaMovies.filter((elem)=>elem.director === "Steven Spielberg" && elem.genre.includes ("Drama"));
    return Spieldramas.length
}
...
function dramaMoviesScore(moviesArray) {
    ...
    let dramasMovies = moviesArray.filter((elem)=>elem.genre.includes ("Drama")) 
    let conteoDramaMovies = dramasMovies.reduce (function(acumulador,movie){
    return acumulador + movie.score / dramasMovies.length
    
    },0)
    return Math.round(conteoDramaMovies*100)/100   
}
...
function turnHoursToMinutes(moviesArray) {
    let durationMovies = 0;
    let newArray = [...moviesArray]
    newArray.map((elem)=>{
        return durationMovies = elem.duration
    })
    console.log (durationMovies[0]*60)
  }
...
function bestYearAvg(moviesArray) {
    ...
    let newArray = [...moviesArray]
    for (let i =0;i<moviesArray.length;i++){
        newArray.push (moviesArray.filter((movie)=>{ 
            return movie.year === moviesArray[i].year;
        })
        )
    }
}
